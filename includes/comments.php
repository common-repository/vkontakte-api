	...
	public function register_settings()
    {
        ...
        add_settings_field('vkapi_appid', // id
            ...
            array(
                ...
                'type' => 'number',
                'descr' => 'Ваше приложение должно иметь тип "Веб-Сайт", введите его id в поле выше, <a href="https://youtu.be/2FJMksrz8Z8" target="_blank">пример создания приложения</a>.',
            ) // args
        );
            ...
            case 'vk':
                switch( $_POST['job'] ) {
                    ...
                    case 'remove':
                        ...
                        $hash = md5(get_option('vkapi_api_secret') . sanitize_text_field($_POST['date']) . sanitize_text_field($_POST['num']) . sanitize_text_field($_POST['last_comment']));

                        if( $hash !== $_POST['sign'] ) {
                            exit('-1');
                        }

                        update_post_meta($_POST['id'], 'vkapi_comm', sanitize_text_field($_POST['num']), false);
            ...
            case 'fb':
                switch( $_POST['job'] ) {
                    case 'add':
                        ...
                        update_post_meta( $post->ID, 'fbapi_comm', (int) $num, false);
